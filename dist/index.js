...
const update_image_1 = __nccwpck_require__(2891);
...
try {
    ...
    const results = update_image_1.updateImage(fs.readFileSync(manifestPath, "utf-8"), containerName, newTag);
    if (multiDocError === "fail" && results.find((result) => result.error)) {
        ...
        results.forEach((result) => {
            ...
            if (result.error && multiDocError !== "ignore") {
                core.warning((_a = result.error) === null || _a === void 0 ? void 0 : _a.message);
            }
...
function findContainer(manifest, containerName) {
    const images = manifest === null || manifest === void 0 ? void 0 : manifest.getIn([
        "spec",
        "template",
        "spec",
        "containers",
    ...
    const img = images;
    const matches = img.items.filter((imageSpec) => imageSpec.get("name") === containerName);
    if (matches.length !== 1) {
...
function updateImage(manifestContent, containerName, newTag) {
    const manifests = Yaml.parseAllDocuments(manifestContent);
    ...
    manifests.map((manifest) => {
        try {
            ...
            if (typeof image !== "string") {
                manifest.error = new Error(`Image of container '${containerName}' is not a string.`);
            }
            else {
